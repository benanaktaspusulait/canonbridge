...
export class MappingsComponent {
  ...
  readonly filtered = computed(() => {
    ...
    if (!q) return this._mappings();
    return this._mappings().filter(m =>
      m.partner.includes(q) || m.eventType.includes(q) || m.version.toLowerCase().includes(q)
    );
  });
  ...
  deprecate(mapping: MappingVersion, event: Event): void {
    this.confirmation.confirm({
      ...
      accept: () => {
        ...
        if (selected?.id === mapping.id) this.selectedMapping.set({ ...selected, status: 'deprecated' });
      }
    });
  }
