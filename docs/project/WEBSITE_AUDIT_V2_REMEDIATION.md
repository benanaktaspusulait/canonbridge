...
| N2 | Done | Removed public webhook exposure. The website posts to a same-origin Next API endpoint with rate limit, honeypot/min-delay validation, optional Turnstile verification, and secret upstream webhook forwarding. |
...
| N14 | Done | Expanded JSON-LD with `Organization`, `SoftwareApplication`, and `FAQPage`; removed the single-item breadcrumb graph. |
| N15 | Done | Generated `canonbridge-og.png` and switched Open Graph/Twitter metadata from SVG to PNG. |
| N16 | Done | Switched the website to Next standalone output and kept manual AVIF/WebP responsive source sets for the large hero product screenshot. |
